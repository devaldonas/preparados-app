  ...
  useEffect(() => {
    const getUser = async () => {
      try {
        ...
        if (profileError) console.log('⚠️ Erro no perfil:', profileError)
        ...
        // SE FOR PARCEIRO - REDIRECIONAR IMEDIATAMENTE
        ...
        console.log('📦 Dashboard - Parceiro (tabela):', partner)
        ...
        if (partner?.status === 'pending') {
          if (typeof window !== 'undefined') {
            window.location.href = '/parceiro/aguardando-aprovacao'
          }
          return
        }
        ...
        if (!hasAccess) {
          console.log('❌ Sem assinatura - redirecionando para planos')
          router.push('/planos')
          return
        }
